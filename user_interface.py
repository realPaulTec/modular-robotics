...
class UserInterface:
    ...
    def __init__(self, MAX_DISTANCE_METERS, ACQUISITION_RADIUS, ACQUISITION_DISTANCE):
        # Tracking constants
    ...
    def update_clusters(self, tracking, tracked_point, prediction, clusters, override):
        # Tracking variables
    ...
    def plot_clusters(self):
        for label, cluster_data in self.clusters.items():
            ...
            # Making clusters not currently considered transparent
            alpha = 0.2 if len(self.tracked_point) > 0 and cluster_data['trackable'] == False else 1

            # Show individual points
            if self.show_hits == True:
                cluster_distances, cluster_angles  = zip(*cluster_data['points'])
                self.axis.scatter(cluster_angles, cluster_distances, s=12, marker='o', alpha=alpha)
           
            # Visualizing central position
            self.axis.scatter(*reversed(cluster_data['central_position']), s=60, c="#FFD700", marker='*')

            # Setting up the label
            label = f"{round(cluster_data['composite_distance'], 4)}"

            # Annotating Bhattacharyya distance for each cluster
            self.axis.annotate(label, reversed(cluster_data['central_position']), color='white')
    ...
    def draw_tracked_point(self):
        ...
        # Drawing acquisition circle
            ...
            color = 'yellow' if not self.override else 'red'
            circle = plt.Circle((self.ACQUISITION_DISTANCE, 0.0), self.ACQUISITION_RADIUS, transform=self.axis.transData._b, color=color, fill=False)
    ...
    def draw_prediction_vector(self):
        ...
        if len(self.prediction) > 0 and len(self.tracked_point) > 0:
            ...
            arrow = plt.arrow(x_track, y_track, fac*(self.prediction[0]-x_track), fac*(self.prediction[1]-y_track), width=0.002, length_includes_head=True, transform=self.axis.transData._b, color='red')
