...
videoDimensions = (video.get(cv2.CAP_PROP_FRAME_WIDTH), video.get(cv2.CAP_PROP_FRAME_HEIGHT))
refined_components = []
acquisitionRadius = 100

targetChannel = 0

def imageColorProcessing(frame, colorChannel):
    # Splitting BGR image
...
def componentProcessing(imageToProcess, whiteImage):
...
def componentProcessingCLAHE(imageToProcess, whiteImage):
    #CLAHE operations
...
def circualarMasking(frame, coordinates, radius):
    # Initialising mask 
    ...
    cv2.circle(frame, coordinates, radius, (0, 0, 255), 4)
...
def adjustContrast(frame, contrast, brightness):
    alpha = contrast
    beta = brightness
...
while(True):
    ...
    if len(refined_components) > 0:
        # Using first track coordinates as mask coordinates
        (x, y) = refined_components[0][1]
        coordinates = (int(x), int(y))
        
        # Using first track dimensions as mask radius
        (width, height) = refined_components[0][2]
        radius = int((width + height) / 2)
    else:
        # Using video center as mask coordinates 
        coordinates = (int(videoDimensions[0] / 2), int(videoDimensions[1] / 2))
        
        # Using standard acquisition radius as mask radius
        radius = acquisitionRadius
    
    # Stage one color processing
    frame = adjustSaturation(frame, 1.2)
    frame = adjustContrast(frame, 1.02, 0)

    # Applying mask with my own function
    maskedFrame = circualarMasking(frame, coordinates, radius)

    # Color processing the image with saturation and my custom colorProcessing function
    colorProcessedImage, whiteImage = imageColorProcessing(maskedFrame, targetChannel)

    # Tracking the induvidual components in an array
    refined_components, out = componentProcessing(colorProcessedImage, whiteImage)
    print(refined_components)
...
# Closing the video channel and windows when quitting the software
video.release()
